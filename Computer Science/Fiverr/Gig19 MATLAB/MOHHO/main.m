%MainMOHHO.m – the constraints are defined here. It calls all functions. All files/functions must be in same folder as the MainMOHHO.m.
clc;
clear;
close all;
%% Problem Definition
...
%lb=[Alb Blb Clb Dlb];
%ub=[Aub Bub Cub Dub];
VarMin=[Alb Blb Clb Dlb]; % Lower Bound of Variables
VarMax=[Aub Bub Cub Dub]; 
%VarMin=10; % Lower Bound of Variables
%VarMax=1520; % Upper Bound of Variables
%% MOPSO Parameters
...
%Sols = [];
for i=1:numel(rep)
 %disp(['solutions = ' num2str(rep.Location(i))]);
 %disp(['costs = ' num2str(rep.Cost(i))]);
 %disp(['Sols = ' num2str(rep.Sol(i))]);
%solutions = rep.Location;
%costs = rep.Cost ;
%Sols = rep.Sol;
solutions = cat(1, solutions, rep.Location);
costs = cat(1, costs, rep.Cost);
%Sols = cat(1, Sols, rep.Sol);
end
