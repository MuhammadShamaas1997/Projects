...
# Define reference point (z1^r, z2^r)
param z1_ref; 
param z2_ref;
param z1_min;
param z2_max;
...
# Define lambda variable for scalarization
var Lambda >= 0;

# Scalarization constraint using a single reference point
subject to Scalarization:
    (z1 - z1_ref) / (z1_ref - z1_min) + (z2 - z2_ref) / (z2_max - z2_ref) <= Lambda;

# Objective: Minimize Lambda to move toward the reference point
minimize z: Lambda;
