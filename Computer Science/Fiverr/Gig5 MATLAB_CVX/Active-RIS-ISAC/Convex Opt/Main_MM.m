...
%sigma2=sqrt(noisepow);
sigmar=sqrt(noisepow);%the noise power of the DFRC BS
sigmaz=sqrt(noisepow);%the noise power of the UE
epsilon=10;% QoS of UE
%% simulation setting
Radius=10;
OutNmax=2;%outer iteration times t_max
Nmax=2;%inner iteration times for BS t_max^1
InNmax=2;%inner iteration times for RIS t_max^2
...
for simul=1:simulationtime  
    ...
    Phi=diag(v1');%N*N
    W=zeros(M,K); %Communication Beamformer
    r=zeros(M,M); %Sensing Beamformer 
    %%%% In the considered scenario, we found that the introduction of the sensing beamformer
    ...
    Wt=zeros(M,K); %iterative Communication Beamformer
    rt=zeros(M,M); %iterative Sensing Beamformer
    d=zeros(1,K);
    e=0;
    iterout=0;
