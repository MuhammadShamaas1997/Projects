...
end_date = pd.Timestamp('2023-01-02 00:00:00')
...
M_HPH = pd.read_excel('./Heureshivernales.xlsx', sheet_name="2023")
# print(M_HPH)
...
# Read an Excel file
data_pv = pd.read_excel('DonnesPV.xlsx')
...
# Read an Excel file
data_wt = pd.read_excel('DonnesWT.xlsx')
# print(data_pv)
# Combine DataFrames by the 'Date' column
...
data_ec = pd.read_excel('DonnesEC.xlsx')
# print(data_ec)
# Combine DataFrames by the 'Date' column
...
try:
    ...
    model.variables.add(names=["x_" + str(i) for i in indices], types=['B'] * len(indices))
    
    # y[t] = 1 if batteries are discharged at time t, 0 otherwise
    model.variables.add(names=["y_" + str(i) for i in indices], types=['B'] * len(indices))
    
    # P_ch_b[t] = Power for battery charging at time t (W)
    model.variables.add(names=["P_ch_b_" + str(i) for i in indices], types=['C'] * len(indices))
    
    # P_dch_b[t] = Power for battery discharging at time t (W)
    model.variables.add(names=["P_dch_b_" + str(i) for i in indices], types=['C'] * len(indices))
    
    # SOC_b[t] = State of charge of the battery at time t (%)
    model.variables.add(names=["SOC_b_" + str(i) for i in indices], types=['C'] * len(indices))

    # Constraints
    for t in indices:
        # Ensure SOC_b is within the limits
        model.linear_constraints.add(
            lin_expr=[[["SOC_b_" + str(t)], [1]]],
            senses=["G"],
            rhs=[SOC_min_b],
            names=["SOC_min_b_" + str(t)]
        )
        ...
        model.linear_constraints.add(
            ...
            senses=["L"],
            rhs=[SOC_max_b],
            names=["SOC_max_b_" + str(t)]
        )

        # Battery can only be charged or discharged, not both at the same time
        model.linear_constraints.add(
            lin_expr=[[["x_" + str(t), "y_" + str(t)], [1, 1]]],
            senses=["L"],
            rhs=[1],
            names=["Charge_Discharge_" + str(t)]
        )

        # Ensure power values are non-negative
        model.linear_constraints.add(
            lin_expr=[[["P_ch_b_" + str(t)], [1]]],
            senses=["G"],
            rhs=[0],
            names=["P_ch_b_nonnegative_" + str(t)]
        )
        
        model.linear_constraints.add(
            lin_expr=[[["P_dch_b_" + str(t)], [1]]],
            senses=["G"],
            rhs=[0],
            names=["P_dch_b_nonnegative_" + str(t)]
        )

    # Initial SOC of the battery
    model.linear_constraints.add(
        lin_expr=[[["SOC_b_" + str(indices[0])], [1]]],
        senses=["E"],
        rhs=[SOC_max_b],
        names=["Initial_SOC_b"]
    )
    ...
    total_renewable_power = list(zip(["P_ch_b_" + str(i) for i in indices] + ["P_dch_b_" + str(i) for i in indices],
                                             [1] * len(indices) + [1] * len(indices)))
    model.objective.set_linear(total_renewable_power)

    # Solve the model
    model.solve()

    # Extract the optimized SOC values and add them to the DataFrame
    optimized_SOC_b = {t: model.solution.get_values("SOC_b_" + str(t)) for t in indices}
    dataframe['Optimized_SOC_b (%)'] = dataframe.index.map(optimized_SOC_b)

    # Export the DataFrame to an Excel file
    dataframe.to_excel('Optimized_Table_CPLEX.xlsx', index=False)
    print("Optimization results exported to 'Optimized_Table_CPLEX.xlsx'")
